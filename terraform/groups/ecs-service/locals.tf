...
locals {
  ...
  container_port             = "8080"
  eric_port                  = "10000"
  docker_repo                = "document-generator"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  lb_listener_rule_priority  = 80
  lb_listener_paths          = ["/private/documents/generate"]
  healthcheck_path           = "/document-generator/healthcheck" #healthcheck path for document generator delivery service
  healthcheck_matcher        = "200"
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "document-generator.env"
