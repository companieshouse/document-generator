...
public class PageRetrieverService<T> {
     ...
     */
    public T retrieve(PageRetrieverClient<T> client,
            final String uri,
                            final ApiClient apiClient,
                            final int itemsPerPage) throws ApiErrorResponseException, URIValidationException {
        int startIndex = 0;
        final T api = client.retrievePage(uri, apiClient, startIndex, itemsPerPage);

        while (/*api.getItems().size()*/ client.getSize(api) < /*api.getTotalResults()*/ client.getTotalCount(api)) {
            startIndex += itemsPerPage;
            final T moreResults = client.retrievePage(uri, apiClient, startIndex, itemsPerPage);
            /* api.getItems().addAll(moreResults.getItems());*/ client.addPage(api, moreResults);
        }
...
//     * @throws URIValidationException should the URI provided prove to be invalid
//     */
//    private T retrievePage(final String uri,
//                                 final ApiClient apiClient,
