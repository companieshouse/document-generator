...
/**
 * The Prosecution module of Document Generator. When a request with a matching
 * URI is received by Document Generator, it is forwarded to this module (see
 * matching, below). The URI is further processed by DocumentGenerator to see
 * which kind of document is to be generated. Different documents require
 * different template variables and so have different methods to fill in those
 * template values and other metadata in a {@link DocumentInfoResponse} that is
 * returned to Document Generator. Document Generator will then use this
 * metadata to determine where to send the template values and other metadata.
 * </p>
 * 
 *
 * <h3>Matching</h3> The enum
 * uk.gov.companieshouse.document.generator.api.document.DocumentType, within
 * the document generator api project, is used to determine whether or not to
 * steer a request to this service, based on whether the URI matches the regex
 * in
 * uk.gov.companieshouse.document.generator.api.document.DocumentType.PROSECUTION.
 * </br>
 ...
 * </li>
 * </ul>
 * This service will be matched on the base of this URI, will use the base to
 * further determine exactly which document type to produce, and then will use
 * the rest of the URI to get the data that it needs.
 */
@Service
public class ProsecutionDocumentInfoService implements DocumentInfoService {
	...
	/**
	 * Entry point from Document Generator, asking for a DocumentInfoResponse that
	 * contains info to build a document, such info will include the location of any
	 * template, any variables that are needed by the template, etc - see
	 * DocumentInfoResponse for more info.
	 */
	@Override
		...
		if (resourceUri.endsWith("ultimatum")) {
			try {
				...
				LOG.errorContext(requestId, String.format("An error occurred when retrieving data for ultimatum"), he,
						debugMap);
				LOG.error("Unmatchable resourceUri inside prosecution: " + docGenUri, debugMap);
