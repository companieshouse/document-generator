...
dependency-check-local: build package
	mvn install -DskipTests
	docker run --rm -e DEPENDENCY_CHECK_SUPPRESSIONS_HOME=/opt/dependency-check-suppressions -v "$$(pwd)":/app -w /app 416670754337.dkr.ecr.eu-west-2.amazonaws.com/dependency-check-runner --repo-name=document-generator
